...
function PatientDetails({ ...props }) {
    ...
    function exportReport() {
        ...
        var _patientId = splitedData[0];
        var _patientName = splitedData[1];
        var _days_from = splitedData[2];
        var _days_to = splitedData[3];
        var _headerLabel = splitedData[4];
        var params = {
            reportName: "Account Recievable - Patient Details",
            Patient: _patientId + "||" + _patientName,
            ...
            if (result.success && result.data != null) {
                setPatientDetailsRowsData(
                    result.data.map((item, i) => {
                        item.column6 = <LinkS style={{marginLeft: 0}} onClick={() => handleOpenDemographics(item.column25)}>{item.column6}</LinkS>
                        return { ...item }
                    }))
            }
        });
                                            ...
                                            patientDetailsRowsData?.map((item, i) => {
                                                return <>
                                                    <tr key={i} className={classes.mainRow}>
                                                        ...
                                                        <td colSpan={1} >{item.column7}</td>
                                                        <td colSpan={1} >{item.column8}</td>
                                                        ...
                                                        <td colSpan={1} className={classes.rightAligned}>{formatCurrency(item.column16)}</td>
                                                        <td colSpan={1} >{item.column21}</td>
                                                        <td colSpan={1} className={classes.balanceDueTd}>{item.column22}</td>
                                                    </tr>
                                                    {item?.children?.map((child, j) => {
                                                        console.log('child', child)
                                                        return (
                                                            <tr key={j}>
                                                                <td colSpan={1}>{child.column9}</td>
                                                                <td colSpan={5}>{child.column10}</td>
                                                                <td colSpan={1}>{child.column11}</td>
                                                                <td colSpan={2}>{child.column12}</td>
                                                                <td colSpan={1} className={classes.rightAligned}>{child.column9}</td>
                                                                ...
                                                                <td colSpan={1} className={classes.rightAligned}>{formatCurrency(child.column19)}</td>
                                                                <td colSpan={1}></td>
                                                                <td colSpan={1}></td>
                                                            </tr>
                                                        )
