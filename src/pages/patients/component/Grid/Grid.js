...
import {Button, Icon,Typography,Grid,Avatar} from "@material-ui/core"
    ...
    const loadData=()=>{
            setRowsData(
                ScheduleSlots.map((item,i)=>{
                    ...
                    item.statusCode=   
                                    <>
                                        <Icon> <img style={{cursor:"pointer"}} src={Erase} onClick={()=>onEdit(item[gridCnfg["ScheduleSlots"][0].field])} className={classes.Icon}/> </Icon>
                                        <Icon> <img style={{cursor:"pointer"}}  src={Delete} onClick={()=>onDelete(item[gridCnfg["ScheduleSlots"][0].field])} className={classes.Icon}/> </Icon>
                                    </>
                                    return {...item}
                }))
    }
                    ...
                    <Grid container direction="row" xs={12} sm={12} md={12} lg={12} xl={12}>
                        <Grid container justify="flex-start" direction="column" xs={5} sm={5} md={5} lg={5} xl={5}>
                            {/* {onAddNew ? 
                                ...
                                {Title ?  <Typography className={classes.title}>Medication Info.</Typography> : ""}
                        ...
                        <Grid container justify="flex-end" direction="row" xs={7} sm={7} md={7} lg={7} xl={7}>
                                 ...
                                 className={classes.BtnIcon} src={PrintIcon}/>} className={classes.cardButton}/>
                                <Button
                                    size="small"
                                    className={classes.newAddBtn2}
                                    onClick={loadData}
